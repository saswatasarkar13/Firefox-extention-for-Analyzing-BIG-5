...
def preprocess_and_predict(input_text):
    ...
    train_df['status_text'] = train_df['status_text'].apply(lambda x: ' '.join([word for word in x.split() if word not in stopwords.words('english')]))  # Change here
    ...
    inp_txt = ' '.join([word for word in inp_txt.split() if word not in stopwords.words('english')])  # Change here
